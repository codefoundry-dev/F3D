...
async function main() {
  ...
  // ── RFQs ───────────────────────────────────────────────────────────────
  const rfqIds = [];
  const rfqStatuses = ['DRAFT', 'OPEN', 'AWAITING_RESPONSE', 'AWARDED', 'CLOSED'];
  for (let i = 0; i < 5; i++) {
  ...
  // ── Purchase Orders ────────────────────────────────────────────────────
  const poIds = [];
  const poStatuses = ['DRAFT', 'SENT', 'ACKNOWLEDGED', 'SCHEDULED_FOR_DELIVERY', 'DELIVERED'];
  const poTypes = ['STANDARD', 'STANDARD', 'BULK', 'HOLD_FOR_RELEASE', 'STANDARD'];
  for (let i = 0; i < 5; i++) {
    const projectId = i % 2 === 0 ? alphaProject.id : betaProject.id;
    const locationId = i % 2 === 0 ? alphaDeliveryLocation?.id : betaDeliveryLocation?.id;
