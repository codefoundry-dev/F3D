...
import { useRfqEmailLog } from '@forethread/rfq-shared';
...
/**
 * Per-RFQ outbound email delivery log (FOR-213): a table of every email sent for
 * the RFQ with its rolled-up delivery status, send time and open count. Bounced /
 * complained / failed deliveries raise a sender-facing alert at the top of the
 * tab and flag the offending rows so the bounce reason is visible.
 */
export function RfqEmailLogTab({ rfqId }: RfqEmailLogTabProps) {
  const { t } = useTranslation('rfqs');
  const { data: entries, isLoading, isError } = useRfqEmailLog(rfqId);
