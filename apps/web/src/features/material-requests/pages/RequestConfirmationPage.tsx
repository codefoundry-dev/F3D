    ...
    >
      <div className="flex flex-col gap-6 px-4 py-6">
        ...
        <div className="flex flex-col items-center gap-4 pt-4 text-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-full bg-[#1B1D22] text-white">
            <CheckCircleIcon className="h-8 w-8" />
          </span>
          <div>
            <p className="text-lg font-semibold text-[#1B1D22]">{t('confirmation.submitted')}</p>
            <p className="text-sm text-[#525866]">{t('confirmation.submittedHint')}</p>
          </div>
        </div>

        {/* Reference id */}
        <div className="rounded-lg bg-[#F4F4F6] px-4 py-3 text-center">
          <p className="text-xs text-[#525866]">{t('confirmation.referenceId')}</p>
          <p className="text-lg font-medium text-[#1B1D22]" data-testid="mr-reference-id">
            {mr.mrNumber}
          </p>
        </div>
        ...
        {/* Request summary */}
        <div className="flex flex-col gap-3">
          <p className="text-sm font-medium text-[#1B1D22]">{t('confirmation.summaryTitle')}</p>
          <div className="flex flex-col gap-3 rounded-lg border border-[#E8EAED] p-4">
            <SummaryRow icon={<ProjectsIcon className="h-4 w-4" />} label={t('confirmation.job')}>
              <span className="text-sm text-[#1B1D22]">{mr.project.name}</span>
            </SummaryRow>
            <SummaryRow
              icon={<PackageIcon className="h-4 w-4" />}
              label={t('confirmation.totalItems')}
            >
              <span className="text-sm text-[#1B1D22]">
            ...
            <SummaryRow icon={<FlagIcon className="h-4 w-4" />} label={t('confirmation.priority')}>
              <span className="text-sm text-[#1B1D22]">{t(priorityKey as never)}</span>
            </SummaryRow>
          </div>
        </div>
        ...
        {/* What happens next */}
        <div className="flex flex-col gap-3">
          <h4 className="text-sm font-medium text-[#1B1D22]">{t('confirmation.whatNext')}</h4>
          <ul className="flex flex-col gap-3">
            {['next1', 'next2', 'next3'].map((key) => (
              <li key={key} className="flex items-start gap-3">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#F4F4F6] text-[#40454F]">
                  <CheckCircleIcon className="h-3.5 w-3.5" />
                </span>
                <span className="text-sm text-[#525866]">{t(`confirmation.${key}` as never)}</span>
              </li>
            ))}
          </ul>
        </div>
