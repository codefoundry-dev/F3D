  ...
  return (
    <MobileShell
      ...
      footer={
        <button
          ...
          className="flex h-[51px] w-full items-center justify-center gap-2 rounded-lg bg-[#171717] text-base text-white hover:bg-[#262626]"
    ...
    >
      <div className="flex flex-col gap-1 px-4 py-6">
        <p className="text-sm text-[#525252]">{t('myRequests.subtitle')}</p>

        {isError ? (
          <div className="px-1 py-12 text-center text-sm text-[#737373]">
          ...
          <div className="flex flex-col items-center gap-1 py-16 text-center">
            ...
            <p className="text-sm font-medium text-[#171717]">{t('myRequests.empty')}</p>
            <p className="text-xs text-[#737373]">{t('myRequests.emptyHint')}</p>
          </div>
        ) : (
...
function RequestRow({ mr, statusLabel }: { mr: MrListItem; statusLabel: string }) {
  const { t } = useTranslation('materialRequests');
  const tone = STATUS_TONE[mr.status] ?? 'bg-[#F5F5F5] text-[#525252]';
  return (
    <li>
      <div className="flex items-start gap-3 rounded-lg border border-[#E5E5E5] px-4 py-3">
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="truncate text-sm font-medium text-[#171717]">{mr.mrNumber}</span>
            <span className={`rounded px-1.5 py-0.5 text-[11px] ${tone}`}>{statusLabel}</span>
          </div>
          <span className="truncate text-xs text-[#525252]">{mr.project.name}</span>
        ...
        <ChevronRightIcon className="mt-1 h-4 w-4 shrink-0 text-[#A3A3A3]" />
      </div>
    </li>
  );
}
