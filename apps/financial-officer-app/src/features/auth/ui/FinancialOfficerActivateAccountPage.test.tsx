...
import FinancialOfficerActivateAccountPage from './FinancialOfficerActivateAccountPage';

describe('FinancialOfficerActivateAccountPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('shows missing token message when no token', () => {
    ...
    render(<FinancialOfficerActivateAccountPage />);
  ...
  it('shows error content when activate mutation fails', () => {
    mockActivateMutation.value = { ...mockActivateMutation.value, isError: true };
    render(<FinancialOfficerActivateAccountPage />);
    expect(screen.getByTestId('reset-password-form')).toBeInTheDocument();
  });

  it('disables request button when no email in expired view', () => {
    mockTokenQuery.value = {
      isLoading: false,
      isError: false,
      data: { valid: false, email: '' },
    };
    render(<FinancialOfficerActivateAccountPage />);
    expect(screen.getByText('invitationExpiredTitle')).toBeInTheDocument();
  });
