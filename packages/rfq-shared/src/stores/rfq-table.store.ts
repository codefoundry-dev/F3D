...
export interface AdvancedFilters {
  ...
  deadlineFrom: string;
  deadlineTo: string;
  minApprovedQuotes: string;
  minApprovedVendors: string;
}
...
export interface RfqTableState {
  /* ─── Table / filter state ─── */
  page: number;
  pageSize: number;
  search: string;
...
export const createRfqTableStore = (defaultColumns: string[], tableName = 'rfqs') =>
  create<RfqTableState>((set, get) => ({
    ...
    applyView: (viewId) => {
      ...
      set({
        ...
        sortBy: view.sortBy ?? 'deadlineRange',
