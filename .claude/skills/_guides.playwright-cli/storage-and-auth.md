...
> **When to use**: Managing cookies, localStorage, sessionStorage, browser storage state, saving and
> restoring authentication, or testing storage-dependent features. **Prerequisites**:
...
The most powerful feature — save the entire browser state (cookies + localStorage for all origins)
to a JSON file, then restore it later to skip login flows.
...
- **Avoid saving state on shared machines** — storage state files contain session tokens and
  personal data
