...
> **When to use**: Running multiple isolated browser sessions concurrently, managing persistent
> profiles, comparing different browser states side by side, parallel scraping, or A/B testing
> flows. **Prerequisites**: [core-commands.md](core-commands.md) for basic CLI usage
...
By default, sessions run **in-memory** — all cookies, storage, and browsing data are lost when the
session closes. Use `--persistent` to save state to disk.
