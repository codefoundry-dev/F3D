...
> dynamic fields, and auto-complete interactions. **Prerequisites**:
> [core/locators.md](locators.md), [core/assertions-and-waiting.md](assertions-and-waiting.md)
...
select, checkbox, radio. **Avoid when**: Never. This is the foundation pattern.
...
`<input type="datetime-local">`, or third-party date pickers. **Avoid when**: The date picker is a
simple text field with no special input type. Just use `fill()`.
...
**Avoid when**: You only care about the happy path. Validation tests should complement, not replace,
success path tests.
...
validation rules. **Avoid when**: The validation is purely server-side with no client-side feedback.
...
validation. **Avoid when**: The form is a single page. Use the basic form filling pattern.
...
suggestions as the user types. **Avoid when**: The field is a plain text input with no suggestions.
...
**Use when**: Form fields appear, disappear, or change based on the value of other fields. **Avoid
when**: All fields are always visible. Use the basic form filling pattern.
...
responses from the server, and loading states during submission. **Avoid when**: You only care about
client-side validation. Test submission separately from validation.
...
default values. **Avoid when**: The form has no reset mechanism.
...
- [core/error-and-edge-cases.md](error-and-edge-cases.md) -- testing form error states and edge
  cases
- [core/accessibility.md](accessibility.md) -- ensuring forms are accessible (label associations,
  ARIA attributes)
