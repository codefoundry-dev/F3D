...
> **When to use**: Testing how your application behaves on phones, tablets, and across viewport
> sizes. Covers device emulation, touch interactions, geolocation, orientation changes, and
> mobile-specific UI patterns. **Prerequisites**: [core/configuration.md](configuration.md),
...
**Use when**: Testing your app as it appears on a specific real-world device -- iPhone, Pixel, iPad.
Applies the correct viewport, user agent, device scale factor, and touch support in one shot.
**Avoid when**: You only need to test a specific viewport width (use custom viewports instead).
Device emulation is not a substitute for real device testing when pixel-perfect rendering matters.
...
**Use when**: Testing responsive layouts at specific breakpoints without full device emulation.
Ideal for verifying CSS media queries fire at the right widths. **Avoid when**: You need realistic
mobile behavior (touch events, mobile user agent, device scale factor). Use device emulation
instead.
...
**Use when**: Testing touch-specific interactions -- tap, swipe, pinch. Required for mobile-only
gestures that have no mouse equivalent. **Avoid when**: The feature works identically with mouse
clicks. Playwright's `click()` dispatches touch events automatically on touch-enabled device
profiles.
...
**Use when**: Testing UI components that only appear on mobile -- hamburger menus, bottom sheets,
pull-to-refresh, sticky mobile headers, floating action buttons. **Avoid when**: The component
...
**Use when**: Testing location-dependent features -- store finders, delivery zones, weather widgets,
location-based pricing. **Avoid when**: The feature does not use the Geolocation API. If it uses
IP-based location, mock the API response instead.
...
**Use when**: Running the same tests across desktop and mobile browsers in parallel. The standard
approach for responsive apps. **Avoid when**: Your app is desktop-only or mobile-only.
...
**Use when**: Systematically verifying layout behavior at every major CSS breakpoint. Best used
alongside visual regression testing. **Avoid when**: You only need one or two viewport sizes -- just
use `test.use()` overrides instead.
...
**Use when**: Testing portrait vs landscape layouts. Critical for tablet apps, media players,
dashboards, and any app that adapts to orientation. **Avoid when**: Your app does not change layout
based on orientation (purely responsive to width only -- test with custom viewports instead).
...
**Use when**: Simulating real-world mobile conditions -- slow 3G networks, underpowered CPUs.
Critical for testing loading states, skeleton screens, and timeout handling. **Avoid when**: Testing
functional correctness only. Performance throttling slows down your test suite significantly.
...
**Use when**: Testing Progressive Web App features -- service workers, install prompts, offline
mode, push notifications on mobile. **Avoid when**: Your app is not a PWA. For general offline
testing, see the network offline pattern in Pattern 9.
...
**Cause**: The browser context was created without `hasTouch: true`. Device profiles set this
automatically, but custom viewport configurations do not.
...
**Cause**: `isMobile` is set by the device profile's `isMobile` property, not by viewport size.
Custom viewports do not set it.
...
**Cause**: Missing `permissions: ['geolocation']` in context options. The browser silently denies
the Geolocation API without this permission.
...
**Cause**: `page.context().newCDPSession(page)` is Chromium-only. Firefox and WebKit do not support
CDP.
...
**Cause**: `page.setViewportSize()` changes the viewport but does not trigger `resize` or
`orientationchange` events in some frameworks that rely on JavaScript-based responsive logic rather
...
**Cause**: Service workers require HTTPS or localhost. Playwright's default `baseURL` of
`http://localhost:3000` works, but other HTTP origins do not.
...
- [core/configuration.md](configuration.md) -- project setup with device profiles and multi-project
  config
- [core/network-mocking.md](network-mocking.md) -- mock API responses alongside mobile testing
- [core/service-workers-and-pwa.md](service-workers-and-pwa.md) -- in-depth PWA testing beyond
  mobile context
- [core/performance-testing.md](performance-testing.md) -- comprehensive performance testing
  including mobile metrics
- [core/browser-apis.md](browser-apis.md) -- geolocation, permissions, and other browser APIs in
  detail
- [ci/projects-and-dependencies.md](../ci/projects-and-dependencies.md) -- advanced multi-project
  patterns for responsive testing matrices
