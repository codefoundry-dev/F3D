...
> **When to use**: Validating your application's defenses against common web vulnerabilities — XSS,
> CSRF, insecure cookies, missing headers, authentication bypass, and sensitive data exposure.
> Playwright is not a replacement for dedicated security scanners, but it catches the most common
> issues as part of your E2E suite. **Prerequisites**:
...
**Avoid when**: You need comprehensive XSS scanning — use a dedicated tool like OWASP ZAP alongside
Playwright.
...
**Use when**: Ensuring state-changing requests include valid CSRF tokens and the server rejects
requests without them. **Avoid when**: Your API uses token-based auth (JWT) with no cookie-based
sessions — CSRF is not applicable.
...
**Use when**: Verifying session cookies and auth cookies have proper security attributes. **Avoid
when**: Your app is fully stateless with no cookies.
...
**Use when**: Ensuring protected routes redirect unauthenticated users and that session invalidation
works. **Avoid when**: Auth is tested through dedicated API tests that cover these cases already.
...
**Use when**: Verifying that HTTP requests are redirected to HTTPS and that sensitive data is not
leaked in URLs, headers, or client-side storage. **Avoid when**: Running against `localhost` where
...
**Use when**: Ensuring the session ID changes after authentication to prevent session fixation
attacks. **Avoid when**: Using stateless token auth (JWT) with no server-side sessions.
...
- [core/third-party-integrations.md](third-party-integrations.md) -- testing OAuth and third-party
  auth providers
