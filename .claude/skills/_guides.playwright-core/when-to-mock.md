...
> **When to use**: When deciding whether to mock API calls, intercept network requests, or hit real
> services in your Playwright tests. **Prerequisites**: [core/locators.md](locators.md),
> [core/assertions-and-waiting.md](assertions-and-waiting.md)

## Quick Answer

**Mock at the boundary, test your stack end-to-end.** Mock third-party services you do not own
(Stripe, SendGrid, OAuth providers, analytics). Never mock your own frontend-to-backend
communication. Your tests should prove that YOUR code works, not that third-party APIs are up.
...
**Use when**: You want to completely replace a third-party API response. The most common mocking
strategy.
...
**Use when**: You want the real API call to happen but need to tweak the response -- injecting error
states, adding edge-case data, or overriding a single field.
...
**Use when**: Complex API sequences with many endpoints (OAuth flows, multi-step wizards, dashboard
data loading). Record once from a real session, replay deterministically. Update the recording
periodically so mocks do not drift from reality.
...
5. Scope HAR to specific URL patterns (`url: '**/api/v2/**'`) so unrelated requests still hit real
   servers.

### Blocking Unwanted Requests

**Use when**: Third-party scripts (analytics, ads, chat widgets) slow down tests and add no value.
Block them outright.
...
**Use when**: You have a shared staging environment that mirrors production. Best for integration
confidence.
...
**Use when**: You need a fully isolated environment with databases, caches, and services. Best for
...
The strongest test suites combine real and mocked services. The principle: **mock what you do not
own, run what you do.**

### Fixture-Based Mock Control

Create a fixture that lets individual tests opt into mocking specific services while keeping
...
Split test projects by environment to run mocked tests in every CI push and full-integration tests
nightly.
